...
- `$ARGUMENTS` contains "skills" → Read `../skills/coach/references/checklists/skills.md`, audit skills only
- `$ARGUMENTS` contains "hooks" → Read `../skills/coach/references/checklists/hooks.md`, audit hooks only
- `$ARGUMENTS` contains "rules" → Read `../skills/coach/references/checklists/rules.md`, audit rules only
- `$ARGUMENTS` contains "memory" → Read `../skills/coach/references/checklists/memory.md`, audit memory only
- `$ARGUMENTS` contains "permissions" → Read `../skills/coach/references/checklists/permissions.md`, audit permissions only
- `$ARGUMENTS` contains "plugins" → Read `../skills/coach/references/checklists/plugins.md`, audit plugins only

### Health mode (cross-cutting analysis)
- `$ARGUMENTS` contains "health" → Read `../skills/coach/references/checklists/usage-stale-friction.md`. Run stale config detection, cross-reference analysis, friction detection, and trend analysis.

### CLAUDE.md modes (operate on CLAUDE.md + rules)
- `$ARGUMENTS` contains "init" → Scaffold a minimal CLAUDE.md (fast, no deep analysis). Read `../skills/coach/references/claude-md-optimize.md` section "Init Mode".
- `$ARGUMENTS` contains "optimize" or "claude-md" → Deep CLAUDE.md + rules cleanup (self-update, bloat detection, rules audit, rewrite). Read `../skills/coach/references/claude-md-optimize.md` section "Optimize Mode".
- `$ARGUMENTS` contains "discover" → Mine codebase + code reviews for conventions → generate `.claude/rules/`. Read `../skills/coach/references/discover.md`.

### Default (full audit)
- No arguments or "all" → Full audit across all areas. Read ALL checklist files in `../skills/coach/references/checklists/`. If deep CLAUDE.md optimization is overdue (see Phase 0), suggest running it.
...
1. Read `.claude/coach-history.md` in the **project directory** — extract:
   ...
   - If `.claude/coach-history.md` doesn't exist, also check `../skills/coach/references/history.md` as legacy fallback — if found there, migrate it to `.claude/coach-history.md` and leave a pointer in the old location
...
For each area in scope, read the relevant checklist(s) from `../skills/coach/references/checklists/` and compare against current state. Identify:
...
- `../skills/coach/references/checklists/skills.md` — Skills gap detection
- `../skills/coach/references/checklists/hooks.md` — Hook purpose coverage
- `../skills/coach/references/checklists/rules.md` — Rule quality checks
- `../skills/coach/references/checklists/memory.md` — Memory completeness + broken reference detection
- `../skills/coach/references/checklists/permissions.md` — Permission discovery from usage
- `../skills/coach/references/checklists/plugins.md` — Plugin coverage
- `../skills/coach/references/checklists/claude-md.md` — CLAUDE.md quality checks
- `../skills/coach/references/checklists/usage-stale-friction.md` — Usage patterns, stale config, cross-reference, friction (always read for full audit and health mode)

Deep-dive references (read only when the matching mode is triggered):
- `../skills/coach/references/claude-md-optimize.md` — Full CLAUDE.md optimization pipeline (self-update, bloat, scan, generate, report, guardrails)
- `../skills/coach/references/rules-audit.md` — Rules audit process (frontmatter, redundancy, pertinence scoring, ROI trimming)
- `../skills/coach/references/discover.md` — Convention discovery from codebase + code reviews
